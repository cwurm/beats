...
func (pInfo ProcessInfo) toMapStr() common.MapStr {
	...
	if pInfo.Username != "" {
		mapstr.Put("user.name", pInfo.Username)
	}
...
func processEvent(pInfo *ProcessInfo, eventType string, eventAction string) mb.Event {
	...
	if pInfo.Username != "" {
		event.RootFields.Put("user.name", pInfo.Username)
	}
...
func (ms *MetricSet) getProcessInfos() ([]*ProcessInfo, error) {
	...
	for _, pid := range pids {
		...
		userAccount, err := user.LookupId(strconv.Itoa(pInfo.UID))
		if err != nil {
			return nil, errors.Wrapf(err, "error looking up process UID")
		}
		//pInfo.Username = userAccount.Username
